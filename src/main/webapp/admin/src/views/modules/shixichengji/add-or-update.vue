<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="实习结果" prop="shixijieguo">
          <el-select v-model="ruleForm.shixijieguo" placeholder="请选择实习结果">
            <el-option
                v-for="(item,index) in shixijieguoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="实习结果" prop="shixijieguo">
	      <el-input v-model="ruleForm.shixijieguo"
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.chengji = true;
          ...
          if(o=='shixijieguo'){
            this.ruleForm.shixijieguo = obj[o];
	    this.ro.shixijieguo = true;
            continue;
          }
          if(o=='beizhu'){
      ...
      this.$http({
		...
		if(json.jiaoshixingming!=''&&json.jiaoshixingming){
          ...
          this.$message.error(data.msg);
            ...
            this.shixijieguoOptions = "未完成,已完成".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `shixichengji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `shixichengji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.shixichengjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
